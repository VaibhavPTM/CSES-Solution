...
private:
    ...
    void updateRangeUtil(int node, int start, int end, int l, int r, int addend) {
        if (lazy[node] != 0) {
            tree[node] += lazy[node];
            if (start != end) {
        ...
        if (start >= l && end <= r) {
            tree[node] += addend;
            if (start != end) {
                lazy[2 * node + 1] += addend;
                lazy[2 * node + 2] += addend;
            }
            return;
        }

        int mid = (start + end) / 2;
        updateRangeUtil(2 * node + 1, start, mid, l, r, addend);
        updateRangeUtil(2 * node + 2, mid + 1, end, l, r, addend);
        tree[node] = max(tree[2 * node + 1], tree[2 * node + 2]);
    }
...
public:
    ...
    void updateRange(int l, int r, int addend) {
        updateRangeUtil(0, 0, n - 1, l, r, addend);
    }
...
void solve() {
    ...
    for(int i = 0; i < q; i++) {
        ...
        if(t == 1) {
            ...
            int curr = ar[idx];
            ar[idx] = val;
            st.updateRange(idx, n-1, -curr);
            st.updateRange(idx, n-1, val);
        }
        else {
            int l, r;
            cin >> l >> r;
            ...
            int curr = 0;
            if(l != 0) {
                curr = st.queryRange(l-1, l-1);
            }
            st.updateRange(l, r, -curr);
            print(max(0ll, st.queryRange(l, r)));
            st.updateRange(l, r, curr);
        }
    }
}
