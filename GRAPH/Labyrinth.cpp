...
// BFS on grid and maintain parant maxtix of grid
#include<bits/stdc++.h>
...
string res;
int n, m; 
string bfs(int x, int y, int dx, int dy, vs &ar) {
    int n = ar.size();
    int m = ar[0].size();
    ...
    while(q.size() > 0) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            if(NX >= 0 && NY >= 0 && NX < n && NY < m && (ar[NX][NY] == '.' || ar[NX][NY] == 'B') && vis[NX][NY] == 0){
                vis[NX][NY] = 1;
                q.push({NX, NY});
