...
int dp[N];
int n, m;
void dfs1(int node, vector<int> &out) {
    vis[node] = 1;
    for(int ch : g1[node]) {
        if(!vis[ch]){
            dfs1(ch, out);
...
void dfs2(int node, vector<int> &res, int id, int &w) {
    vis[node] = 1;
    res[node-1] = id;
    w += cost[node];
    for(int ch : g2[node]) {
...
int dfs3(int node, map<int,int> &cost) {
    int mx = cost[node];
    int t = 0;
    if(dp[node] != -1) return dp[node];
    for(int ch : g3[node]) {
        t = max(t, dfs3(ch, cost));
    }
    return dp[node] = mx + t;
}
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        for(int node : g1[i]) {
            if(res[i-1] != res[node-1]) {
                g3[res[i-1]].pb(res[node-1]);
                in[res[node-1]]++;
            }
        }
    }
