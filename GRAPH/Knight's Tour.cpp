...
void sieve();
const int N = 8, total_move = 64;
vector<vector<int>> res(N+1, vector<int> (N+1));
vector<int> xx = {-2, -2, 2, 2, -1, 1, -1, 1};
vector<int> yy = {1, -1, -1, 1, -2, -2, 2, 2};
vector<int> getD(int x, int y) {
    int cnt = 0;
    for(int i = 0; i < xx.size(); i++) {
        int X = x + xx[i];
        int Y = y + yy[i];
        if(X >= 1 && Y >= 1 && X <= N && Y <= N && res[X][Y] == 0) {
            cnt++;
        }
    }
    return {cnt, x, y};
}
bool backtrack(int x, int y, int move) {
    if(move > N * N) {
        return true;
    }
    ...
    for(auto i : tmp) {
        res[i[1]][i[2]] = move;
        if(backtrack(i[1], i[2], move + 1)) return 1;
        res[i[1]][i[2]] = 0;
    }
    return false;
}
