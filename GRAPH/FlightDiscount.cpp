...
void solve() {
    ...
    vi dis1 = dijkstra(0, g);
    vi dis2 = dijkstra(n-1, g1);
    int res = INF;
    for(int i = 0; i < m; i++) {
        int a = tmp[i][0], b = tmp[i][1], w = tmp[i][2];
        // print(a, b, dis1[a], dis2[b], w);
        int tres = dis1[a] + dis2[b] + (w / 2);
        res = min(res, tres);
    }
    print(res);
}
