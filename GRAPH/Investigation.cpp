...
void solve() {
    ...
    vi dist(n+1, INF), mxe(n+1), mne(n+1), way(n+1), vis(n+1);
    minhp(p(int,int)) pq;
    pq.push({0, 1});
    way[1] = 1;
    dist[1] = 0;
    ...
    while(pq.size() > 0) {
        ...
        for(auto ch : g[node]) {
            int childe = ch.ff, w = ch.ss;
            if(weight + w < dist[childe]) {
                dist[childe] = weight + w;
                pq.push({dist[childe], childe});
                way[childe] = way[node];
                mxe[childe] = mxe[node] + 1;
                mne[childe] = mne[node] + 1; 

            }
            else if(weight + w == dist[childe]) {
                way[childe] = (way[childe] + way[node]) % MOD;
                mxe[childe] = max(mxe[childe], mxe[node] + 1);
                mne[childe] = min(mne[childe], mne[node] + 1);
            }
        }
        vis[node] = 1;
    }   

    print(dist[n], way[n], mne[n], mxe[n]);
