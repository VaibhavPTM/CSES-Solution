...
int n, m;
vector<int> g[N];
bool dfs(int node, int par, vector<int> &vis, vector<int> &st, vector<int> &parant) {
    parant[node] = par;
    vis[node] = 1;
    st[node] = 1;
    for(int ch : g[node]) {
        if(vis[ch] == 0) {
            int f = dfs(ch, node, vis, st, parant);
            if(f) return true;
        }
        else if(st[ch]) {
            vector<int> res;
...
void solve() {
    ...
    vector<int> vis(n+1), st(n+1), par(n+1);
    for(int i = 1; i <= n; i++) {
        if(vis[i] == 0 && dfs(i, i, vis, st, par)) {
            return;
        }
    }
    print("IMPOSSIBLE");
