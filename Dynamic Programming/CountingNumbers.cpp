...
int help(int i, int prev, int leding_zero, int t, string str) {
    if(i == str.size()) {
        return 1;
    }
    if(prev != -1 && dp[i][prev][leding_zero][t] != -1) return dp[i][prev][leding_zero][t];
    int res = 0;
    int lb = 0;
    int up = (t ? str[i]-'0':9);
    for(int d = lb; d <= up; d++) {
        if(d == prev && d != 0) continue;
        if(d == prev && d == 0 && leding_zero == 0) continue;
        res += help(i+1, d,leding_zero&(d==0), t & (d == up) ,str);
    }
    return dp[i][prev][leding_zero][t] = res;
}

int getans(int a) {
    string str = to_string(a);
    memset(dp, -1, sizeof dp);
    return help(0, -1, 1, 1, str);
}
