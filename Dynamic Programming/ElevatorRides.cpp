...
void solve() {
    ...
    int dp1[1<<n],dp2[1<<n];
    // dp1[mask] = f[mask] = rides
    // dp2[mask]= g[mask] = weight

    dp1[0]=1,dp2[0]=0;
    for (int mask = 1; mask < (1ll << n); mask++) {
        dp1[mask]=n,dp2[mask]=0;
        for (int y = 0; y < n; y++) {
            if ((mask) & (1ll<<y)) {
                int prev_mask = mask ^ (1ll << y);
                int prev_weight=dp2[prev_mask];
                int prev_ride=dp1[prev_mask];
                int curr_weight,curr_ride;
                if(prev_weight+weight[y]<=x){
                    curr_weight=prev_weight+weight[y];
                    curr_ride=prev_ride;
                }
                else{
                    curr_weight=weight[y];
                    curr_ride=prev_ride+1;
                }
                if(dp1[mask]==curr_ride) dp2[mask]=min(dp2[mask], curr_weight);
                else dp2[mask]=curr_weight;
                dp1[mask] = min(dp1[mask], curr_ride);
            }
        }
    }
    cout << dp1[(1<<n)-1]<<endl;
