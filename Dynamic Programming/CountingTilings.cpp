...
int dp[1001][(1 << 10)];
int n, m;
void genNewMask(int i, vi &masks, int prev_mask, int new_mask) {
    if(i == (n)) {
        masks.pb(new_mask);
        return;
    }
    if(prev_mask & (1 << i)) {
        genNewMask(i+1, masks, prev_mask, new_mask);

    }
    else {
        if(i != (n-1) && (prev_mask & (1 << (i+1))) == 0) {
            genNewMask(i+2, masks, prev_mask, new_mask);
        }
        genNewMask(i+1, masks, prev_mask, new_mask + (1 << i));
    }

}
int help(int i, int mask) {
    if(i == (m+1)) {
        if(!mask)
            return 1;
        else return 0;
    ...
    int res = 0;
    vi new_mask;
    genNewMask(0, new_mask, mask, 0);
    for(int nm : new_mask) {
        res = mod_add(res, help(i+1, nm));
    }
    new_mask.clear();
