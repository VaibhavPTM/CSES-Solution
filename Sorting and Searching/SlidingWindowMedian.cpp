...
v(int) ys = {1,-1,0,0};
void sieve();
void print(ordered_multiset st) {
    for(auto i : st) p0(i);
    print("");
}
void solve(){
    ...
    while(r < n) {
        // print(st);
        int sz = st.size() / 2 - (st.size() % 2 == 0);
        res.pb(*st.find_by_order(sz));
        // print(*st.find_by_order(st.order_of_key(ar[l])));
        st.erase(st.find_by_order(st.order_of_key(ar[l])));
        // print(st);
        st.insert(ar[r]);
        l++;
        r++;
