...
void solve() {
    ...
    // lastFree
    ordered_set av;
    map<int,vector<int>> avmp;
    for(int i = 0; i < n*2; i++) {
        if(ar[i].ss > 0) {
            auto IT = av.order_of_key(ar[i].ff);
            if(IT == 0) {
                res[ar[i].ss-1] = curr;
                mp[ar[i].ss] = curr;
                used.insert(curr);
                ...
                auto it = av.find_by_order(IT-1);
                res[ar[i].ss-1] = avmp[*it].back();
                mp[ar[i].ss] = avmp[*it].back();
                used.insert(avmp[*it].back());
                avmp[*it].pop_back();
                av.erase(it);
            }
        }
        else {
            // print(ar[i].ff, ar[i].ss, mp[abs(ar[i].ss)]);
            av.insert(ar[i].ff);
            avmp[ar[i].ff].pb(mp[abs(ar[i].ss)]);
            used.erase(mp[abs(ar[i].ss)]);
            mp.erase(ar[i].ss);
        }
    }
    print(curr-1);
